...
# function Laplacian_circle!(P,A,Δx)
#     B = CircularArray(A);
#     for R ∈ CartesianIndices(P)
#         P[R] = (-B[R[1]+2,R[2]] + 16B[R[1]+1,R[2]] 
#         + 16B[R[1]-1,R[2]] -B[R[1]-2,R[2]] 
#         -B[R[1],R[2]+2] + 16B[R[1],R[2]+1] 
#         + 16B[R[1],R[2]-1] -B[R[1],R[2]-2] 
#         - 60B[R[1],R[2]]) /(12Δx^2);
...
function update_7!(N,A₁,A₂,Ȧ₁,Ȧ₂,M₁,M₂,ω,η,Δx,Δt,time)
    ...
    Ȧ₁ = Ȧ₁ .+ 0.5Δt .* (F₁ .+ L₁ .- a(lime).^β * λ .* B₁ .* (B₁.^2 .+ B₂.^2 .- η^2) .- α * © .* Ȧ₁ ./ lime)
    Ȧ₂ = Ȧ₂ .+ 0.5Δt .* (F₂ .+ L₂ .- a(lime).^β * λ .* B₂ .* (B₁.^2 .+ B₂.^2 .- η^2) .- α * © .* Ȧ₂ ./ lime)
...
function saving_laplace!(N,t₀,t,A₁,A₂,Ȧ₁,Ȧ₂,ω,η,Δx,Δt)
    ...
    update_7!(N, A₁, A₂, Ȧ₁, Ȧ₂, M₁, M₂, ω, η, Δx, Δt, time);
